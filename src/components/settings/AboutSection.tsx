...
export function AboutSection({ isPortable }: AboutSectionProps) {
  ...
  const handleOpenReleaseNotes = useCallback(async () => {
    try {
      ...
      await settingsApi.openExternal(
        `https://github.com/StudyingPy/cc-switch-connectioncheck/releases/tag/${displayVersion}`,
      );
    } catch (error) {
  ...
  const handleOpenGithub = useCallback(() => {
    void settingsApi.openExternal(
      "https://github.com/StudyingPy/cc-switch-connectioncheck",
    );
  }, []);
      ...
      >
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <p className="min-w-0 flex-1 text-xs leading-relaxed sm:text-right">
            <a
              href="https://github.com/StudyingPy/cc-switch-connectioncheck"
              onClick={(event) => {
